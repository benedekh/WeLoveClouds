...
public class HelpMessageGenerator {
    ...
    buffer.append("|registerGet <key> | <key>: The key that indexes the desired value (Max length 20 bytes). | retrieves the value for the given key from the server. | Returns the value for the given key or an error if the key does not exist in storage.|\n");
    ...
    buffer.append("|registerPut <key> <value> | <key>: Arbitrary string used to identify the <value> (max length 20 bytes).");
