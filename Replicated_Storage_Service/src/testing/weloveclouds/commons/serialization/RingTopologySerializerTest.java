...
/**
 * Created by Benoit on 2017-01-03.
 */
...
public class RingTopologySerializerTest {
    ...
    public void setUp() throws Exception {
        storageNodeHashRange = new HashRange.Builder().begin(Hash.MIN_VALUE)
                .end(Hash.MAX_VALUE).build();
        childHashRange1 = storageNodeHashRange;
        childHashRange2 = storageNodeHashRange;

        replica1 = new StorageNode.Builder()
                .name(REPLICA1_NAME)
                .serverConnectionInfo(new ServerConnectionInfo.Builder().ipAddress
                        (REPLICA1_IP).port(REPLICA1_PORT).build())
                .build();

        replica2 = new StorageNode.Builder()
                .name(REPLICA2_NAME)
                .serverConnectionInfo(new ServerConnectionInfo.Builder().ipAddress
                        (REPLICA2_IP).port(REPLICA2_PORT).build())
                .build();

        serviceEndpoint1 = new ServerConnectionInfo.Builder()
                .ipAddress(SERVICE_IP_1)
                .port(SERVICE_PORT_1)
                .build();
        serviceEndpoint2 = new ServerConnectionInfo.Builder()
                .ipAddress(SERVICE_IP_2)
                .port(SERVICE_PORT_2)
                .build();

        serviceHealthInfos1 = new ServiceHealthInfos.Builder()
                .serviceName(SERVICE_NAME_1)
                .servicePriority(SERVICE_PRIORITY_1)
                .serviceEnpoint(serviceEndpoint1)
                .serviceStatus(SERVICE_STATUS_1)
                .numberOfActiveConnections(NUMBER_OF_ACTIVE_CONNECTIONS_1)
                .build();
        serviceHealthInfos2 = new ServiceHealthInfos.Builder()
                .serviceName(SERVICE_NAME_2)
                .servicePriority(SERVICE_PRIORITY_2)
                .serviceEnpoint(serviceEndpoint2)
                .serviceStatus(SERVICE_STATUS_2)
                .numberOfActiveConnections(NUMBER_OF_ACTIVE_CONNECTIONS_2)
                .build();

        nodeHealthInfos1 = new NodeHealthInfos.Builder()
                .nodeName(NODE_NAME)
                .nodeStatus(NODE_STATUS)
                .addServiceHealtInfos(serviceHealthInfos1)
                .build();

        nodeHealthInfos2 = new NodeHealthInfos.Builder()
                .nodeName(NODE_NAME)
                .nodeStatus(NODE_STATUS)
                .addServiceHealtInfos(serviceHealthInfos2)
                .build();

        storageNode1 = new StorageNode.Builder()
                .name(NODE_NAME)
                .serverConnectionInfo(new ServerConnectionInfo.Builder().ipAddress
                        (NODE_IP_1).port(NODE_PORT_1).build())
                .hashRange(storageNodeHashRange)
                .childHashRanges(Arrays.asList(childHashRange1, childHashRange2))
                .healthInfos(nodeHealthInfos1)
                .replicas(Arrays.asList(replica1, replica2))
                .build();

        storageNode2 = new StorageNode.Builder()
                .name(NODE_NAME_2)
                .serverConnectionInfo(new ServerConnectionInfo.Builder().ipAddress
                        (NODE_IP_2).port(NODE_PORT_2).build())
                .hashRange(storageNodeHashRange)
                .childHashRanges(Arrays.asList(childHashRange1, childHashRange2))
                .healthInfos(nodeHealthInfos2)
                .replicas(Arrays.asList(replica1, replica2))
                .build();
        ...
        ringTopologyDeserializer = new RingTopologyDeserializer<>(new StorageNodeDeserializer(new
                ServerConnectionInfoDeserializer(), new HashRangeDeserializer(), new
                NodeHealtInfosDeserializer(new ServiceHealthInfosDeserializer(new
    ...
    public void shouldSerializeValidRingTopology() throws Exception {
        ...
        RingTopology<StorageNode> deserializeRingTopology = ringTopologyDeserializer.deserialize
                (serializedRingTopology.toString());
        SerializedMessage serializedMessage = new KVEcsNotificationMessageSerializer
                (ringTopologySerializer, nodeHealthInfosSerializer).serialize(new
                KVEcsNotificationMessage.Builder().status(IKVEcsNotificationMessage.Status.TOPOLOGY_UPDATE).ringTopology
                (ringTopologyMock)
                .build());
        IKVEcsNotificationMessage deserializedMessage = new KVEcsNotificationMessageDeserializer
                (new RingTopologyDeserializer<>(new StorageNodeDeserializer(new
                        ServerConnectionInfoDeserializer(), new HashRangeDeserializer(), new
                        NodeHealtInfosDeserializer(new ServiceHealthInfosDeserializer(new ServerConnectionInfoDeserializer())
                ))), new NodeHealtInfosDeserializer(new ServiceHealthInfosDeserializer(new
                        ServerConnectionInfoDeserializer()))).deserialize(serializedMessage);
